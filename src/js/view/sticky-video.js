...
const stickyVideo = function ( $scope, $ ) {
    ...
    var overlay  = element.data( 'overlay' )
    ...
    playerAbc.on( 'play', function ( event ) {
        element.closest( '.eael-sticky-video-player2' ).
            find( '.plyr__poster' ).
            hide()
	...
	if ( isEditMode ) {
		elementor.hooks.addAction(
			...
			function ( panel, model, view ) {
				var interval
				model.attributes.settings.on( 'change:eaelsv_sticky_width',
					function () {
				...
				model.attributes.settings.on(
					...
					function () {
						...
						interval = setTimeout( function () {
							var width = Math.ceil(
								model.getSetting( 'eaelsv_sticky_height' ) *
								1.78,
							)
	...
	if ( scrollBottom > jQuery( window ).height() + 400 ) {
		if ( scrollTop >= eaelsvDomHeight ) {
			if ( videoIsActive == 'on' ) {
				...
				PositionStickyPlayer( eaelsvPosition, eaelsvHeight,
					eaelsvWidth )
			}
		} else {
